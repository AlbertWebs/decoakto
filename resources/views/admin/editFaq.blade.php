...
@section('content')
	...
	<form action="{{ url('/admin/edit_Faq/'.$FAQ->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
		@csrf
		<div class="rounded-xl bg-white ring-1 ring-gray-200 p-4 space-y-4">
			<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
				<div>
					<label class="mb-1 block text-sm font-medium text-gray-700">Title</label>
					<input name="title" required value="{{ old('title', $FAQ->title) }}" class="w-full rounded-md border border-gray-200 bg-white px-3 py-2 text-sm text-gray-800" />
				</div>
				<div>
					...
					<select name="category" class="w-full rounded-md border border-gray-200 bg-white px-3 py-2 text-sm text-gray-800">
						@foreach(($Category ?? []) as $cat)
							<option value="{{ $cat->title }}" {{ (old('category', $FAQ->category)===$cat->title) ? 'selected' : '' }}>{{ $cat->title }}</option>
						@endforeach
					</select>
				</div>
			</div>
			<div>
				...
				<textarea name="meta" rows="3" class="w-full rounded-md border border-gray-200 bg-white px-3 py-2 text-sm text-gray-800">{{ old('meta', $FAQ->meta) }}</textarea>
			</div>
			<div>
				<label class="mb-1 block text-sm font-medium text-gray-700">Content</label>
				<textarea id="content_editor" name="content" rows="10" class="w-full rounded-md border border-gray-200 bg-white px-3 py-2 text-sm text-gray-800">{!! old('content', $FAQ->content) !!}</textarea>
			</div>
			<div class="pt-2">
