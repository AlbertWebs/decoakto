	...
	<div class="mb-5 flex items-center justify-between">
		<div class="flex items-center gap-2">
			<span class="inline-flex h-9 w-9 items-center justify-center rounded-md bg-indigo-50 text-indigo-600 ring-1 ring-indigo-100">
				<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-5 w-5"><path d="M4 6h16v12H4zM6 4h12v2H6z"/></svg>
			</span>
			<h2 class="text-lg font-semibold">Portfolios</h2>
		</div>
		<a href="{{ url('/admin/addPortfolio') }}" class="inline-flex items-center gap-2 rounded-md bg-indigo-600 px-3 py-2 text-sm font-medium text-white hover:bg-indigo-700">
		...
		<div class="overflow-x-auto">
			<table class="min-w-full divide-y divide-gray-200 text-sm">
				...
				<tbody class="divide-y divide-gray-100">
					@forelse(($Portfolio ?? []) as $pf)
						<tr>
							<td class="px-4 py-3">
								@php $img = $pf->image_one ? url('/uploads/portfolios/'.$pf->image_one) : null; @endphp
								@if($img)
									<img src="{{ $img }}" alt="{{ $pf->name }}" class="h-12 w-20 rounded object-cover ring-1 ring-gray-200" />
								@else
							...
							<td class="px-4 py-3 text-gray-800 font-medium">{{ $pf->name }}</td>
						    <td class="px-4 py-3 text-gray-600">{{ $pf->category_title }}</td>
							<td class="px-4 py-3 text-gray-500">{{ $pf->created_at }}</td>
							<td class="px-4 py-3">
								<div class="flex items-center justify-end gap-2">
									<a href="{{ url('/admin/editPortfolios/'.$pf->id) }}" class="inline-flex items-center gap-1 rounded-md border border-gray-200 bg-white px-2 py-1 text-xs text-gray-700 hover:bg-gray-50">
									...
									<a href="{{ url('/admin/deletePortfolio/'.$pf->id) }}" onclick="return confirm('Delete this portfolio?')" class="inline-flex items-center gap-1 rounded-md bg-rose-600 px-2 py-1 text-xs text-white hover:bg-rose-700">
					...
					@empty
						<tr>
							<td class="px-4 py-8 text-center text-gray-500" colspan="5">No portfolios found.</td>
						</tr>
					@endforelse
				</tbody>
			</table>
