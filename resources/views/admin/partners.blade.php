...
@section('content')
	<div class="mb-5 flex items-center justify-between">
		...
		<a href="{{ url('/admin/addPartner') }}" class="inline-flex items-center gap-2 rounded-md bg-indigo-600 px-3 py-2 text-sm font-medium text-white hover:bg-indigo-700">
			...
			<table class="min-w-full divide-y divide-gray-200 text-sm">
				...
				<tbody class="divide-y divide-gray-100">
					@forelse(($Partner ?? []) as $p)
						<tr>
							<td class="px-4 py-3">
								@php $img = $p->image ? url('/uploads/partners/'.$p->image) : null; @endphp
								@if($img)
									<img src="{{ $img }}" alt="{{ $p->name }}" class="h-10 w-10 rounded object-cover ring-1 ring-gray-200" />
								@else
							...
							<td class="px-4 py-3 text-gray-600">{{ $p->slung }}</td>
							<td class="px-4 py-3 text-gray-500">{{ $p->created_at }}</td>
							<td class="px-4 py-3">
								<div class="flex items-center justify-end gap-2">
									<a href="{{ url('/admin/editPartners/'.$p->id) }}" class="inline-flex items-center gap-1 rounded-md border border-gray-200 bg-white px-2 py-1 text-xs text-gray-700 hover:bg-gray-50">
									...
									<a href="{{ url('/admin/deletePartner/'.$p->id) }}" onclick="return confirm('Delete this partner?')" class="inline-flex items-center gap-1 rounded-md bg-rose-600 px-2 py-1 text-xs text-white hover:bg-rose-700">
					...
					@empty
						<tr>
							<td class="px-4 py-8 text-center text-gray-500" colspan="5">No partners found.</td>
						</tr>
					@endforelse
				</tbody>
			</table>
