...
@section('content')
	<div class="mb-5 flex items-center justify-between">
		<div class="flex items-center gap-2">
			...
			<h2 class="text-lg font-semibold">Edit Portfolio</h2>
		</div>
	...
	<form action="{{ url('/admin/edit_Portfolio/'.$Portfolio->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
		@csrf
		...
		<div class="rounded-xl bg-white ring-1 ring-gray-200 p-4 space-y-4">
			...
			<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
				<div>
					...
					<input name="title" required value="{{ old('title', $Portfolio->name) }}" class="w-full rounded-md border border-gray-200 bg-white px-3 py-2 text-sm text-gray-800 focus:ring-2 focus:ring-indigo-100" />
				</div>
				<div>
					...
					<select name="category" class="w-full rounded-md border border-gray-200 bg-white px-3 py-2 text-sm text-gray-800">
						@foreach(($Category ?? []) as $cat)
							<option value="{{ $cat->id }}" {{ (old('category', $Portfolio->category_id)==$cat->id) ? 'selected' : '' }}>{{ $cat->title }}</option>
						@endforeach
					</select>
				</div>
			</div>
			<div>
				...
				<textarea name="meta" rows="3" class="w-full rounded-md border border-gray-200 bg-white px-3 py-2 text-sm text-gray-800">{{ old('meta', $Portfolio->meta) }}</textarea>
			</div>
			<div>
				<label class="mb-1 block text-sm font-medium text-gray-700">Content</label>
				<textarea id="content_editor" name="content" rows="8" class="w-full rounded-md border border-gray-200 bg-white px-3 py-2 text-sm text-gray-800">{!! old('content', $Portfolio->content) !!}</textarea>
			</div>
			<div>
				<label class="mb-1 block text-sm font-medium text-gray-700">Images</label>
				<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
					@foreach(['one','two','three'] as $idx)
						@php $imgField = 'image_'.$idx; @endphp
						<div class="upload-tile group rounded-lg border-2 border-dashed border-gray-200 bg-white p-3 hover:border-indigo-300 cursor-pointer" data-target="input_image_{{ $idx }}">
							<div class="flex items-center gap-3">
								<div class="relative h-24 w-24 overflow-hidden rounded-md bg-gray-100 border-2 border-dashed border-gray-300 flex items-center justify-center">
									<img id="preview_image_{{ $idx }}" src="{{ $Portfolio->$imgField ? url('/uploads/portfolios/'.$Portfolio->$imgField) : '' }}" alt="" class="{{ $Portfolio->$imgField ? '' : 'hidden' }} absolute inset-0 h-full w-full object-cover" />
									<svg id="placeholder_image_{{ $idx }}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="{{ $Portfolio->$imgField ? 'hidden' : '' }} h-6 w-6 text-gray-400"><path d="M21 7.5V18a3 3 0 01-3 3H6a3 3 0 01-3-3V7.5L7.5 3h9L21 7.5z"/></svg>
								</div>
								<div class="flex-1">
								...
								<button type="button" id="clear_image_{{ $idx }}" class="{{ $Portfolio->$imgField ? '' : 'hidden' }} shrink-0 rounded-md border border-gray-200 bg-white px-2 py-1 text-xs text-gray-700 hover:bg-gray-50">Clear</button>
							</div>
						</div>
					@endforeach
				</div>
