	...
	<div class="mb-5 flex items-center justify-between">
		<div class="flex items-center gap-2">
			<span class="inline-flex h-9 w-9 items-center justify-center rounded-md bg-sky-100 text-sky-700 ring-1 ring-sky-200">
				<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-5 w-5"><path d="M12 2a10 10 0 100 20 10 10 0 000-20zM9 10h6v2H9v-2zm0 4h6v2H9v-2z"/></svg>
			</span>
			<h2 class="text-lg font-semibold">Privacy Policies</h2>
		</div>
		<a href="{{ url('/admin/addPrivacy') }}" class="inline-flex items-center gap-2 rounded-md bg-indigo-600 px-3 py-2 text-sm font-medium text-white hover:bg-indigo-700">
		...
		<div class="overflow-x-auto">
			<table class="min-w-full divide-y divide-gray-200 text-sm">
				...
				<tbody class="divide-y divide-gray-100">
						...
						<tr>
							<td class="px-4 py-3 text-gray-800 font-medium">
								<span class="inline-flex items-center gap-2">
									<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-4 w-4 text-sky-600"><path d="M12 2a10 10 0 100 20 10 10 0 000-20z"/></svg>
									{{ $p->title }}
								</span>
							</td>
							...
							<td class="px-4 py-3">
								<div class="flex items-center justify-end gap-2">
									<a href="{{ url('/admin/editPrivacy/'.$p->id) }}" class="inline-flex items-center gap-1 rounded-md border border-gray-200 bg-white px-2 py-1 text-xs text-gray-700 hover:bg-gray-50">
									...
									<a href="{{ url('/admin/delete_privacy/'.$p->id) }}" onclick="return confirm('Delete this privacy policy?')" class="inline-flex items-center gap-1 rounded-md bg-rose-600 px-2 py-1 text-xs text-white hover:bg-rose-700">
					...
					@empty
						<tr>
							<td class="px-4 py-8 text-center text-gray-500" colspan="3">No privacy policies found.</td>
						</tr>
					@endforelse
				</tbody>
			</table>
